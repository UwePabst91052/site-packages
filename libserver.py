...
class Message:
    def __init__(self, selector, sock, addr, parent):
        ...
        self.request = None
        self.response_text = "Request executed successfully"
    ...
    def _create_forwarding_json_content(self):
        action = self.request.get("action")
    ...
    def create_forwarding(self):
        ...
        message = self._create_message(**forward)
        for client in self.client_list:
            if self.addr != client[1]:
                client[2].message_created = True
                client[2].add_to_send_buffer(message)
